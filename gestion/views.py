...
def mostrarCargos(request):
    ...
    if estadoSesion is True:
        if nomUsuario.upper() == "ADMIN":
            cargos = Cargo.objects.all().values().order_by('nombre_cargo')
...
def registrarCargo(request):
    ...
    if estadoSesion is True and nomUsuario.upper() == "ADMIN":
        ...
        if request.method == "POST":
            ...
            nombre_cargo = request.POST["nombre_cargo"].strip()
            
            if Cargo.objects.filter(nombre_cargo=nombre_cargo).exists():
                datos = {'nomUsuario': nomUsuario, 'cargos': cargos, 'r2': 'El cargo ya existe.'}
                return render(request, 'cargos.html', datos)
            elif not nombre_cargo:
                datos = {'nomUsuario': nomUsuario, 'cargos': cargos, 'r2': 'El cargo debe contener un nombre válido.'}
                return render(request, 'cargos.html', datos)
    ...
    try:
        ...
        if estadoSesion is True:
            ...
            if nomUsuario == "ADMIN":
                ...
                if request.method == "POST":                    
                    nuevo_nombre = request.POST["nuevo_nombre"].strip()
                    if not nuevo_nombre:
                        cargos = Cargo.objects.all().order_by('nombre_cargo')
                        datos = {'nomUsuario': nomUsuario, 'cargos': cargos, 'r2': 'El cargo debe contener un nombre válido.'}
                        return render(request, 'cargos.html', datos)
